...
class NM_drone:
    ...
    def translation(self, speed=20, angle=0, distance=50):
        '''
        以任意角度平移
        :param distance: 平移距离 cm
        :param speed: 平移速度 cm/s
    ...
    def move_to_target(self, position, target_position):
        ...
        if distance < 1:  # 如果误差小于0.5米，认为已经到达目标
        ...
        # self.send_control(lr, fb, ud, deg)
        # sleep(10)
        ...
        # receive_nm = ReceivePacket()  # 实例化一个数据包接收类
        # receive_nm.catch_pack()  # 监听主机收到的数据包(仅监听arp数据包)
        # # 计算当前位置与目标位置之间的误差
        # target_x, target_y = target_position
        # current_x, current_y = position
        # distance = math.sqrt((target_x - current_x) ** 2 + (target_y - current_y) ** 2)
